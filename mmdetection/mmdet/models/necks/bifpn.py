...
class BiFPNModule(nn.Module):
    def __init__(self, channels, levels, init=0.5, conv_cfg=None, norm_cfg=None, act_cfg=None):
    ...
    def forward(self, inputs):
        ...
        for i in range(0, levels - 2, 1):
            pathtd[i + 1] = (
                w2[0, i] * inputs[i + 1]
                + w2[1, i] * nn.Upsample(scale_factor=0.5)(pathtd[i])
                + w2[2, i] * pathtd[i + 1]
            )
            pathtd[i + 1] = self.bifpn_convs[jj](pathtd[i + 1])
            jj = jj + 1

        pathtd[levels - 1] = w1[0, kk] * inputs[levels - 1] + w1[1, kk] * nn.Upsample(
            scale_factor=0.5
        )(pathtd[levels - 2])
